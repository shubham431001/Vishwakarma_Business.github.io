<?php
    ...
    if (isset($_POST['send-reset-link'])) {
        $query = "SELECT * FROM `registered_usersss` WHERE `email`='$_POST[email]'";
        ...
        if($result)
        {
            if(mysqli_num_rows($result)==1)
            {
                ...
                $date = date("Y-m-d");
                $query = "UPDATE `registered_usersss` SET `resettoken`='$reset_token',`resettokenexpire`='$date' WHERE `email`='$_POST[email]'";
